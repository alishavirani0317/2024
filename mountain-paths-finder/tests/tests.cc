// clang-format off

// clang-format on
/////////////////////////////////////////////////////////////////////////////////////////////
//                             Framework Set-up //
/////////////////////////////////////////////////////////////////////////////////////////////
#ifdef CATCH_CONFIG_MAIN
#  undef CATCH_CONFIG_MAIN
#endif
#include <algorithm>
#include <fstream>
#include <iostream>
#include <iterator>
#include <string>

#include "catch.hpp"

/////////////////////////////////////////////////////////////////////////////////////////////
//                                 Includes //
/////////////////////////////////////////////////////////////////////////////////////////////
#include <stdexcept>

#include "color.hpp"
#include "elevation_dataset.hpp"
#include "grayscale_image.hpp"
#include "path.hpp"
#include "path_image.hpp"

/////////////////////////////////////////////////////////////////////////////////////////////
//                             Helpers/Constants Declarations //
/////////////////////////////////////////////////////////////////////////////////////////////
bool CompareFiles(const std::string& p1, const std::string& p2);

/////////////////////////////////////////////////////////////////////////////////////////////
//                                Color Examples //
/////////////////////////////////////////////////////////////////////////////////////////////
TEST_CASE("Example: Color initialization", "[example_color_init]") {
  SECTION("Invalid red component") {
    REQUIRE_THROWS(Color(256, 128, 128));  // NOLINT
    REQUIRE_THROWS(Color(-1, 128, 128));   // NOLINT
  }
}
// mine
TEST_CASE("color initialization", "[color]") {
  SECTION("valid color components") {
    REQUIRE_NOTHROW(Color(0, 0, 0));        // black
    REQUIRE_NOTHROW(Color(255, 255, 255));  // white
    REQUIRE_NOTHROW(Color(128, 128, 128));  // gray
  }

  SECTION("invalid color components", "[color]") {
    // Out of range values
    REQUIRE_THROWS(Color(256, 128, 128));  // red component out of range
    REQUIRE_THROWS(Color(-1, 128, 128));   // red component out of range
    REQUIRE_THROWS(Color(128, 256, 128));  // green component out of range
    REQUIRE_THROWS(Color(128, -1, 128));   // green component out of range
    REQUIRE_THROWS(Color(128, 128, 256));  // blue component out of range
    REQUIRE_THROWS(Color(128, 128, -1));   // blue component out of range

    // edge cases
    REQUIRE_NOTHROW(Color(0, 0, 0));        // mins
    REQUIRE_NOTHROW(Color(255, 255, 255));  // maxs
  }
}

TEST_CASE("Equality operator", "[color]") {
  Color black(0, 0, 0);
  Color white(255, 255, 255);
  Color gray(128, 128, 128);

  SECTION("Same colors") {
    REQUIRE(black == black);
    REQUIRE(white == white);
    REQUIRE(gray == gray);
  }

  SECTION("Different colors") {
    REQUIRE_FALSE(black == white);
    REQUIRE_FALSE(white == gray);
    REQUIRE_FALSE(gray == black);
  }
}

/////////////////////////////////////////////////////////////////////////////////////////////
//                                Elevation Data Examples //
/////////////////////////////////////////////////////////////////////////////////////////////

TEST_CASE("Example Elevation Data Test", "[ed-forward-2-3]") {
  const std::string kTestFile = "./example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  const std::vector<std::vector<int>> kElevationDataGroundTruth = {
      {9, 12}, {8, 10}, {7, 14}};
  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  const auto& student_elevation_data = ed.GetData();

  // Are the vectors kElevationDataGroundTruth and student_elevation_data
  // identical? If so, pass test.
  REQUIRE(std::equal(student_elevation_data.begin(),
                     student_elevation_data.end(),
                     kElevationDataGroundTruth.begin()));
}

TEST_CASE("ed_too_little", "[ed_too_little]") {
  const std::string kTestFile = "example-data/all-tie-row1-2w-3h.dat";
  constexpr size_t kTestFileWidth = 100;
  constexpr size_t kTestFileHeight = 100;
  REQUIRE_THROWS(ElevationDataset(kTestFile, kTestFileWidth, kTestFileHeight));
}

TEST_CASE("ed_too_much", "[ed_too_much]") {
  const std::string kTestFile = "example-data/all-tie-row1-2w-3h.dat";
  constexpr size_t kTestFileWidth = 1;
  constexpr size_t kTestFileHeight = 1;
  REQUIRE_THROWS(ElevationDataset(kTestFile, kTestFileWidth, kTestFileHeight));
}

TEST_CASE("elevation_dataset constructor ", "[elevation_dataset]") {
  const std::string kTestFile = "example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  const std::vector<std::vector<int>> kElevationDataGroundTruth = {
      {9, 12}, {8, 10}, {7, 14}};

  // test constructor & data getting
  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  const std::vector<std::vector<int>> kElevationData = ed.GetData();

  // check if data matches ground truth
  REQUIRE(kElevationData == kElevationDataGroundTruth);
}

TEST_CASE("elevation_dataset width & height", "[elevation_dataset]") {
  const std::string kTestFile = "example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;

  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);

  // test width & height functions
  REQUIRE(ed.Width() == kTestFileWidth);
  REQUIRE(ed.Height() == kTestFileHeight);
}

TEST_CASE("elevation_dataset max & min elevation", "[elevation_dataset]") {
  const std::string kTestFile = "example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  constexpr int kMaxEle = 14;
  constexpr int kMinEle = 7;

  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);

  // test MaxEle and MinEle functions
  REQUIRE(ed.MaxEle() == kMaxEle);
  REQUIRE(ed.MinEle() == kMinEle);
}

TEST_CASE("elevation_dataset DatumAt", "[elevation_dataset]") {
  const std::string kTestFile = "example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  constexpr int kDatum1 = 12;
  constexpr int kDatum2 = 8;

  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);

  // test DatumAt function for specific positions
  REQUIRE(ed.DatumAt(0, 1) == kDatum1);
  REQUIRE(ed.DatumAt(1, 0) == kDatum2);
}
/////////////////////////////////////////////////////////////////////////////////////////////
//                              Grayscale Image Examples //
/////////////////////////////////////////////////////////////////////////////////////////////

TEST_CASE("Example Grayscale Data Test", "[gi-forward-2-3]") {
  const std::string kTestFile = "./example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  const std::vector<std::vector<Color>> kImageGroundTruth = {
      {Color(73, 73, 73), Color(182, 182, 182)},
      {Color(36, 36, 36), Color(109, 109, 109)},
      {Color(0, 0, 0), Color(255, 255, 255)}};

  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  GrayscaleImage gi(ed);
  const auto& s_image_data = gi.GetImage();

  // Are the vectors kImageGroundTruth and simage_data
  // identical? If so, pass test.
  REQUIRE(std::equal(s_image_data.begin(),
                     s_image_data.end(),
                     kImageGroundTruth.begin()));
}

TEST_CASE("PPM Example Test: Grayscale image from prompt_5w_2h.ppm",
          "[ppm_ex_grayscale_prompt_5w_2h]") {
  const std::string kOutput =
      "./student_out/ex_grayscale_prompt_5w_2h.ppm";
  const std::string kTestFile = "./example-data/ex_input_data/prompt_5w_2h.dat";
  const std::string kGroundTruth =
      "./example-data/ex_output_grayscale/prompt_5w_2h.ppm";
  constexpr size_t kTestFileWidth = 5;
  constexpr size_t kTestFileHeight = 2;
  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  GrayscaleImage gi(ed);
  gi.ToPpm(kOutput);
  REQUIRE(CompareFiles(kOutput, kGroundTruth));
}

// mine

TEST_CASE("GrayscaleImage Constructor with ElevationDataset",
          "[grayscale image]") {
  const std::string kTestFile = "./example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  const std::vector<std::vector<Color>> kImageGroundTruth = {
      {Color(73, 73, 73), Color(182, 182, 182)},
      {Color(36, 36, 36), Color(109, 109, 109)},
      {Color(0, 0, 0), Color(255, 255, 255)}};

  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  GrayscaleImage gi(ed);
  const auto& image_data = gi.GetImage();

  // Check if the image data matches the ground truth
  REQUIRE(std::equal( image_data.begin(),
                      image_data.end(),
                     kImageGroundTruth.begin()));
}
// values for groundtruths are not correct
// TEST_CASE("grayscaleimage constructor with file & dimensions",
//           "[grayscale image]") {
//   const std::string kTestFile =
//   "./example-data/ex_input_data/prompt_5w_2h.dat"; constexpr size_t
//   kTestFileWidth = 5; constexpr size_t kTestFileHeight = 2; const
//   std::vector<std::vector<Color>> kImageGroundTruth = {
//       {Color(26, 0, 51),
//        Color(76, 0, 102),
//        Color(128, 0, 153),
//        Color(179, 0, 204),
//        Color(153, 0, 204)},
//       {Color(51, 0, 102),
//        Color(76, 0, 102),
//        Color(102, 0, 102),
//        Color(115, 0, 102),
//        Color(102, 0, 102)}};





TEST_CASE("grayscaleimage width & height", "[grayscale image]") {
  const std::string kTestFile = "./example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;

  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  GrayscaleImage gi(ed);

  // Check if the width and height match the test file dimensions
  REQUIRE(gi.Width() == kTestFileWidth);
  REQUIRE(gi.Height() == kTestFileHeight);
}

TEST_CASE("grayscaleimage maxcolorvalue", "[grayscale image]") {
  const std::string kTestFile = "./example-data/ex_input_data/prompt_5w_2h.dat";

  ElevationDataset ed(kTestFile, 5, 2);
  GrayscaleImage gi(ed);

  // Check if the max color value is 255
  REQUIRE(gi.MaxColorValue() == 255);
}

/////////////////////////////////////////////////////////////////////////////////////////////
//                              Computing Paths: Navigation Examples //
/////////////////////////////////////////////////////////////////////////////////////////////

TEST_CASE("Example Navigation Test: Go forward.", "[forward-2-3]") {
  const std::string kTestFile = "./example-data/ex_input_data/forward-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  const std::vector<size_t> kPathToEvaluateGroundTruth = {1, 1};
  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  GrayscaleImage gi(ed);
  PathImage pi(gi, ed);

  // Let's get the path's vector from `pi`
  const auto& s_paths = pi.Paths();

  // We want to analyze the program's behavior on the path starting from Row 1,
  // so let's get the path we created for that row.
  const auto& s_path_to_evaluate = s_paths.at(1).GetPath();

  // Now let's compare that path to the expected path stored in
  // kPathToEvaluateGroundTruth.
  REQUIRE(std::equal(s_path_to_evaluate.begin(),
                     s_path_to_evaluate.end(),
                     kPathToEvaluateGroundTruth.begin()));
}

TEST_CASE("Example Navigation Test: Go Up.", "[forward-up-2-3]") {
  const std::string kTestFile =
      "./example-data/ex_input_data/forward-up-2-3.dat";
  constexpr size_t kTestFileWidth = 2;
  constexpr size_t kTestFileHeight = 3;
  const std::vector<size_t> kPathToEvaluateGroundTruth = {1, 0};
  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  GrayscaleImage gi(ed);
  PathImage pi(gi, ed);
  const auto& s_paths = pi.Paths();
  const auto& s_path_to_evaluate = s_paths.at(1).GetPath();
  REQUIRE(std::equal(s_path_to_evaluate.begin(),
                     s_path_to_evaluate.end(),
                     kPathToEvaluateGroundTruth.begin()));
}

/////////////////////////////////////////////////////////////////////////////////////////////
//                              Path Image Examples //
/////////////////////////////////////////////////////////////////////////////////////////////

TEST_CASE("Example Test: Path image from prompt_5w_2h.ppm",
          "ex_path_image_prompt_5w_2h]") {
  const std::string kOutput =
      "./student_out/ex_path_image_prompt_5w_2h.ppm";
  const std::string kTestFile = "./example-data/ex_input_data/prompt_5w_2h.dat";
  const std::string kGroundTruth =
      "./example-data/ex_output_paths/prompt_5w_2h.ppm";
  constexpr size_t kTestFileWidth = 5;
  constexpr size_t kTestFileHeight = 2;
  ElevationDataset ed(kTestFile, kTestFileWidth, kTestFileHeight);
  GrayscaleImage gi(ed);
  PathImage pi(gi, ed);
  pi.ToPpm(kOutput);
  REQUIRE(CompareFiles(kOutput, kGroundTruth));
}

/////////////////////////////////////////////////////////////////////////////////////////////
//                             Helper Definitions //
/////////////////////////////////////////////////////////////////////////////////////////////
bool CompareFiles(const std::string& p1, const std::string& p2) {
  std::ifstream f1(p1);
  std::ifstream f2(p2);

  if (f1.fail() || f2.fail()) {
    return false;  // file problem
  }

  std::string f1_read;
  std::string f2_read;
  while (f1.good() || f2.good()) {
    f1 >> f1_read;
    f2 >> f2_read;
    if (f1_read != f2_read || (f1.good() && !f2.good()) ||
        (!f1.good() && f2.good()))
      return false;
  }
  return true;
}

// TEST_CASE("descriptor of what is being tested",
//           "[identifier if we wanted to use just this test cases]") {
//   REQUIRE(Weight(1) == 1);  //if false value then whole thing fails
// }
