...
class KDTree
{
  ...
  public:
    ...
    //left -> idx of first point in range of points
    // right -> idx of last point in range of points 
    KDTreeNode* buildKDTree(vector<Point<Dim>>& points, int left, int right, int dimension);
    /**
    ...
    //helper
    Point<Dim> findNNHelper(KDTreeNode* node, const Point<Dim>& query, int depth) const; 
    // functions used for grading:

    double getDistance(const Point<Dim>& a, const Point<Dim>& b) const;
